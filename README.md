...
1. `AGENTS.md` — AIエージェントが最初に読む共通の入口（必須）
2. `WORK_PROTOCOL.md` — 目的・成果物・確認・記録の共通フロー（必須）
3. `RULES.md` — 禁止事項、確認が必要な操作、秘密情報の扱い（必須）
4. `VERIFICATION.md` — 「できました」と言う前の確認手順（必須）
4. `DESIGN.md` — 見た目の品質が価値に直結する案件で使う（実運用で最も出番が多い1つ）
...
- 最初の実用ツールを小さく作る
- 必要になったファイル（`BUSINESS.md` / `SERVERS.md` / `TASKS.md` / `WORKLOG.md`）だけを後から足す
