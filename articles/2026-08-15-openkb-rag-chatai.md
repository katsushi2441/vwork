...
- **KBが小さいうち（〜数百KB）**: OpenKBを使わず、**生Markdownをそのままコンテキストに渡す**のが最も単純で正確。安価なDeepSeek APIで実測したところ、全カタログ＋制度文書を読ませた状態で**1回答あたり約$0.01（約1.5円）**（実測は2026年8月上旬。8/16のDeepSeek価格改定後は1.5〜5円程度になる計算だが、ナレッジを固定プレフィックスにしてコンテキストキャッシュを効かせれば1円未満に抑えられる）。価格・URLの事実も落とさず、応答速度はローカル12B比で約2.7倍でした
...
- 小さいKBは直渡し＋安価API（DeepSeekで1回答1.5〜5円程度・キャッシュ構成なら1円未満）、大きくなったらPageIndex——この使い分けが2026年時点の現実解

---

**更新履歴**: 2026-08-16 DeepSeekの価格改定（8/16施行・ピーク/オフピーク制導入で従来比約1.5〜5倍）を反映し、コスト記述を更新しました。「小さいKBは直渡し」の設計が有効なことは変わりませんが、**ナレッジ部分を固定プレフィックスにしてコンテキストキャッシュを効かせる**ことが改定後は実質必須になっています（キャッシュヒット時の入力単価はミス時の約1/30）。
