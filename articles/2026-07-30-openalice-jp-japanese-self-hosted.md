...
**小嶋 篤 著 — バイブコーディング×バイブトレーディングで暗号資産・FX戦略を育てる**

OpenAlice-JPのようなAIエージェントを「眺めて終わり」にせず、自分の取引システムを安全に組み立て、検証し、改善していきたい人のための実践入門書です。

完成済みの売買ツールや「儲かる戦略」を配る本ではありません。VPSの契約とSSH接続から始め、Codex CLIと日本語で相談しながら、オープンソースの自動取引システムであるNOFX／kfreqaiと、AI判断APIのkcbrain＋DeepSeekを構築します。自然な言葉で考えた売買アイデアを戦略コードへ落とし込み、バックテストとPaper取引で確かめ、ログを見ながら改善する流れを学べます。

序章＋全49章、Kindle版は518ページ。各章にAIへの依頼文が用意され、プログラミング未経験者でもコマンドの丸暗記ではなく、AIとの対話を通じて次の内容を実践できる構成です。

- AI自動取引を構成する市場データ、戦略、リスク管理、注文実行の理解
- 自分の売買アイデアを検証可能な戦略へ変える手順
- バックテストと、実資金を使わないPaper取引による改善サイクル
- ログ監視、Git管理、バックアップなど継続運用の基礎
- 実資金へ移行する前に確認すべき判断基準

本記事で扱ったOpenAlice-JPとは使用ソフトウェアが異なりますが、「AIに任せきりにせず、画面とログで事実を確認する」「最初は実資金を使わずに検証する」という安全思想は共通しています。AIと一緒に取引システムを作るところから、戦略を育てるところまで一通り実践したい方に適した一冊です。

[Amazon.co.jpで『AIと作る自動取引ボット入門』を見る](https://www.amazon.co.jp/dp/B0HC27BLHG)

※本書はソフトウェア開発と運用学習を目的とする技術書です。特定の金融商品、取引所、売買戦略を推奨するものではなく、投資助言ではありません。暗号資産・FX取引には元本を失うリスクがあります。
