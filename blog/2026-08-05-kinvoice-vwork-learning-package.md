...
- **動くシステム** — 領収書PDFの発行とメール送信。DB不要・外部ライブラリ不要・約1,180行で全部読めます
- **`AGENTS.md`** — AIエージェントに渡すと、設置と初期設定を各段階の確認つきで進めます
...
**私たちが提供しているのは、コードへのアクセスではありません。**どう作って、どこでつまずいて、どう気づいたかです。

AIを使えば、動くものは速く出てきます。そのあとに残る仕事は「本当に動いたのか」を確かめることで、そこには手順があります。それが [VWork](https://exbridge.jp/vwork.html) です。
